...
export default function Home(props: HomeProps) {
  ...
  async function createPool(event: FormEvent) {
    ...
    try {
      ...
      alert(
        "O bolão foi criado e o código do bolão está na área de transferência! "
      );
      setPoolTitle("");
    } catch (error) {
      console.log(error);
      alert("O bolão não foi criado, tente novamente!");
    }
  }
  ...
  return (
    <div className="max-w-[1124px] h-screen mx-auto grid grid-cols-2 items-center gap-28">
      <main>
        <Image src={LogoCopa} alt="Logo do Nlw Copa" />
        <h1 className="mt-14 text-white text-5xl font-bold leading-tight">
          Crie seu próprio bolão da copa e compartilhe entre amigos!
        </h1>
        <div className="mt-10 flex items-center gap-2">
          <Image src={Avatares} alt="Foto participantes do bolão" />
      ...
      <Image src={appImage} alt="App Preview Image" quality={100} />
    </div>
  );
}
